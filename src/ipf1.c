	  ...
	  if ( i != world_rank ) {
	    ...
	    if ( n_xfer > 0 ) {
	      ...
	      MPI_Recv( x, N * state_dim, MPI_DOUBLE, i, 2, MPI_COMM_WORLD, 
			MPI_STATUS_IGNORE );
	      // TODO:
	      // Because within island resampling is done after the inter-island
	      // resampling also the weights need to be sent
	      MPI_Recv( w, N, MPI_DOUBLE, i, 12, MPI_COMM_WORLD, MPI_STATUS_IGNORE );
	...
	for (int i = 0; i < world_size; i++ ) {
	  if( world_rank != i ) {
	    ...
	    if ( n_xfer > 0 ) {
	      ...
	      MPI_Send( w, n_xfer, MPI_DOUBLE, i, 12, MPI_COMM_WORLD );
