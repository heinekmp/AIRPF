      ...
      char *argv[]) 
{
  ...
  int *cube_mtx;
  cube_mtx = malloc( sizeof( double ) * nDim * world_size ); 
  hypercube_matrix( &nthCube, cube_mtx, nDim, world_rank );
  ...
  for( long n = 0; n < data_length; n++ ) {
    ...
    flexCombineEstimates( world_rank, state_dim, estimates, output, n, 
			  output_dim, world_size, &total_w, &my_normal_weight, 
			  &N_steps, epc_threshold, cube_mtx, last_stage );
